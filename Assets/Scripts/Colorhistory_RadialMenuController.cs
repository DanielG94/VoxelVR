// Originaler code von VRTK Plugin, wurde nur minimal abgeändert
using UnityEngine;
using VRTK;
...
/// </example>
[RequireComponent(typeof(Colorhistory_RadialMenu))]
public class Colorhistory_RadialMenuController : MonoBehaviour
{
    [Tooltip("The controller to listen to the controller events on.")]
    public VRTK_ControllerEvents events;

    protected Colorhistory_RadialMenu menu;
    protected float currentAngle; //Keep track of angle for when we click
    protected bool gripTouched;

    protected virtual void Awake()
    {
        menu = GetComponent<Colorhistory_RadialMenu>();
    ...
    protected virtual void DoGripPressed(object sender, ControllerInteractionEventArgs e)
    {
        gripTouched = true;
        DoShowMenu(CalculateAngle(e));
    }

    protected virtual void DoGripReleased(object sender, ControllerInteractionEventArgs e)
    {
        gripTouched = false;
        DoHideMenu(false);
    }
